...
- `androidFakechroot` - Fakechroot with Android glibc RPATH
- `packAuditLib` - pack-audit.so for `/nix/store` path rewriting

**Build options in nix-on-droid-src/modules/build/config.nix:**
- `build.androidGlibc` - Android-patched glibc package
- `build.standardGlibc` - Standard glibc for path redirection
- `build.androidFakechroot` - Android-patched fakechroot
