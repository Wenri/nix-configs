...
let
  ...
  # Absolute prefix for store paths (for Android/proot compatibility)
  storePrefix = if cfg.absoluteStorePrefix != null
    then cfg.absoluteStorePrefix
    else "";

  # Helper to create symlinks with optional prefix
  mkSymlink = target: "${storePrefix}${target}";
  ...
  options = {
    ...
    build = {
      absoluteStorePrefix = mkOption {
        ...
        default = null;
        example = "/data/data/com.termux.nix/files/usr";
        description = ''
          Prefix to add to all /nix/store symlinks. This makes store paths
          accessible both inside and outside proot on Android.
          
          When set, symlinks will point to e.g.
      ...
      activationPackage =
        pkgs.runCommand
          ...
          ''
            ...
            ln --symbolic ${mkSymlink "${config.build.etc}/etc"} $out/etc
            ln --symbolic ${mkSymlink "${config.environment.path}"} $out/nix-on-droid-path

            ln --symbolic ${mkSymlink "${config.environment.files.login}"} $out/filesystem/bin/login
            ln --symbolic ${mkSymlink "${config.environment.files.loginInner}"} $out/filesystem/usr/lib/login-inner
            ln --symbolic ${mkSymlink "${config.environment.files.prootStatic}/bin/proot-static"} $out/filesystem/bin/proot-static

            ln --symbolic ${mkSymlink "${config.environment.binSh}"} $out/filesystem/bin/sh
            ln --symbolic ${mkSymlink "${config.environment.usrBinEnv}"} $out/filesystem/usr/bin/env
          '';
    };
